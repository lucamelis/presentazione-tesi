...
\begin{block}{Public-key cryptography}  
 ...
 \begin{itemize}
  ...
  \item users can decrypt or sign only if  enough cooperate
  \end{itemize}  
 \end{block}
...
\frametitle{ThPKE: Protocol phases}
  ...
  \begin{itemize}
 \item<1-2> \alert<1,2>{ \textbf{Key Generation} }
 \item<3-4> \alert<3,4>{ \textbf{Key Assembly} }
 \item<5-8> \alert<5-8>{ \textbf{Encryption} }
 \item<9-14> \alert<9-14>{ \textbf{Partial Decryption} } 
 \item<15,16> \alert<15,16>{ \textbf{Finish Decryption} }
 \end{itemize} 
    \column{0.50\textwidth}
\begin{overprint}
 \onslide<1,2> \begin{center}\pgfuseimage{key_gen}\end{center}
 \onslide<3,4> \begin{center}\pgfuseimage{key_assembly}\end{center}
 \onslide<5-8> \begin{center}\pgfuseimage{enc}\end{center}
 \onslide<9-14> \begin{center}\pgfuseimage{part_dec}\end{center}
 \onslide<15,16> \begin{center}\pgfuseimage{finish}\end{center}
\end{overprint}
...
\end{block}
 ...
 \begin{block}{Commitment protocol}
  \begin{itemize}[<+->] 
   \item Alice \textbf{commits} the message and Bob does not learn any information about it (\alert{hiding} property)
   \item Alice chooses to \textbf{open} the commitment and reveal the message, but she cannot change the value committed (\alert{binding} property)
  \end{itemize}
 \end{block}
